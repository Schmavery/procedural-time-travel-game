 ...
 * http://www.java-gaming.org/topics/basic-spritebatcher-with
 * -lwjgl/27730/view.html
 */
public class SpriteBatch {
	...
	private static byte getColor(float f) {
		return (byte) (f * 255);
	}
	...
	public void draw(int texID, float x, float y, float sizex, float sizey,
			float rotation, Vector4f col) {
		draw(texID, empty, x, y, sizex, sizey, col);
	}

	public void draw(int texID, float[] region, float x, float y, float width,
			float height, ReadableColor c) {
		draw(texID, region, x, y, width, height, new Vector4f(
				c.getRedByte() / 255f, c.getGreenByte() / 255f,
				c.getBlueByte() / 255f, c.getAlphaByte() / 255f));
	}

	public void draw(int texID, float[] region, float x, float y, float width,
			float height, Vector4f col) {
		if (texID != currentTex) {
			render();
			currentTex = texID;
		}
